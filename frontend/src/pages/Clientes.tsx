...
function ClienteModal({ 
  ...
  useEffect(() => {
    ...
    if (cliente) {
      ...
      setFormData({
        nome: '',
        email: '',
        telefone: clientePreenchido.telefone || '',
        telefone2: '',
        whatsapp: '',
        endereco: '',
        numero: '',
  ...
  useEffect(() => {
    if (Object.keys(erros).length > 0) {
      ...
      if (erros.nome && nomeRef.current) {
        nomeRef.current.focus()
      } else if (erros.cpf_cnpj && cpfCnpjRef.current) {
        cpfCnpjRef.current.focus()
      } else if (erros.telefone && telefoneRef.current) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // CPF/CNPJ e Telefone: pelo menos UM deve estar preenchido
    const cpfCnpjPreenchido = formData.cpf_cnpj && formData.cpf_cnpj.trim()
    const telefonePreenchido = formData.telefone && formData.telefone.trim()
    
    if (!cpfCnpjPreenchido && !telefonePreenchido) {
      // Se nenhum dos dois estiver preenchido, marcar ambos como erro
      validarCampo('cpf_cnpj', '', 'obrigatorio')
