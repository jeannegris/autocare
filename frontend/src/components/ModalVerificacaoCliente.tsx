...
interface ModalVerificacaoClienteProps {
  ...
  onCriarNovo: (cpfCnpj: string, tipo: 'CPF' | 'CNPJ' | 'TELEFONE') => void
...
export default function ModalVerificacaoCliente({
  ...
  const buscarCliente = async () => {
    if (!cpfCnpj.trim()) {
      setErro('Por favor, informe um CPF, CNPJ ou telefone')
      return
    }

    // Detectar o tipo de entrada (CPF, CNPJ ou telefone)
    const apenasNumeros = cpfCnpj.replace(/\D/g, '')
    const temFormatoTelefone = cpfCnpj.includes('(') || cpfCnpj.includes(')') || 
                               (cpfCnpj.includes('-') && cpfCnpj.includes(' ')) ||
                               cpfCnpj.match(/\(\d{2}\)/)
    
    let isTelefone = false
    let tipoDoc: 'CPF' | 'CNPJ' | 'TELEFONE' = 'CPF'

    // Verificar se é telefone (10 ou 11 dígitos com formato de telefone, ou 10 dígitos sem formato)
    if ((apenasNumeros.length === 10 || apenasNumeros.length === 11) && 
        (temFormatoTelefone || apenasNumeros.length === 10)) {
      isTelefone = true
      tipoDoc = 'TELEFONE'
    } else {
      // Validar CPF/CNPJ
    ...
    try {
      ...
      if (isTelefone) {
        // Buscar por telefone
        response = await apiFetch(`/clientes/buscar-telefone/${apenasNumeros}`)
      } else {
        // Buscar por CPF/CNPJ
        response = await apiFetch(`/clientes/buscar-cpf-cnpj/${apenasNumeros}`)
      }
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-blue-700 mb-4">
                  Não encontramos nenhum cliente com o {resultado.tipoDoc === 'TELEFONE' ? 'telefone' : resultado.tipoDoc} informado.
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Informe o CPF, CNPJ ou telefone do cliente
            </label>
            <p className="text-sm text-gray-500 mb-3">
              Digite o documento (CPF/CNPJ) ou número de telefone para verificar se o cliente já está cadastrado no sistema.
            </p>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="000.000.000-00, 00.000.000/0000-00 ou (XX) XXXXX-XXXX"
                maxLength={18}
                disabled={loading}
              />
              <button
