...
export const useValidacao = () => {
  ...
  const validarCampo = (campo: string, valor: string, tipo: 'CPF' | 'CNPJ' | 'telefone' | 'email' | 'cep' | 'placa' | 'obrigatorio' | 'customizado') => {
    ...
    const nomeAmigavel = mensagensAmigaveis[campo] || campo.charAt(0).toUpperCase() + campo.slice(1)
    
    switch (tipo) {
      case 'obrigatorio':
        resultado = valor.trim() ? { valido: true, mensagem: '' } : { valido: false, mensagem: `${nomeAmigavel} é obrigatório` }
        break
      case 'CPF':
